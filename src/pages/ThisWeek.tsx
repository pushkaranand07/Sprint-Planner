...
export default function ThisWeek() {
  ...
  const thisWeekTasks = useMemo(() => {
    return tasks.filter(task => task.sprint === 'this-week');
  }, [tasks]);

  const tasksByPriority = useMemo(() => {
    const urgent = thisWeekTasks.filter(task => task.priority === 'urgent');
    const high = thisWeekTasks.filter(task => task.priority === 'high');
    const other = thisWeekTasks.filter(task => !['urgent', 'high'].includes(task.priority));
  ...
  const stats = useMemo(() => {
    ...
    const overdue = thisWeekTasks.filter(task => 
      task.dueDate && 
            ...
            >
              ...
              <ul className="space-y-3">
                ...
                >
                  ...
                  <span className="text-gray-700">Complete all high-priority tasks</span>
                </motion.li>
                <motion.li
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                ...
                >
                  ...
                  <span className="text-gray-700">Review sprint progress</span>
                </motion.li>
                <motion.li
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                ...
                >
                  <Clock className="w-4 h-4 text-gray-500 mr-3" />
                  <span className="text-gray-700">Plan next week's tasks</span>
                </motion.li>
              </ul>
            </motion.div>
          </div>
